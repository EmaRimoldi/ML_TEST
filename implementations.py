...
def loss_logistic_regression(y, tx, w, class0=1, class1=1):
    '''
    Compute the loss for logistic regression ( Cross-Entropy Loss ). 
    '''
    loss = -np.sum(class0 * y * np.log(sigmoid(np.dot(tx, w))) + class1 * (1 - y) * np.log(1 - sigmoid(np.dot(tx, w)))) / y.shape[0]
    return loss
...
def calculate_gradient_logistic_regression_weights(y, tx, w, class_0_weight, class_1_weight):
    '''
    Compute the gradient for logistic regression considering weights.
    '''
    ...
    # Create a weights array based on the true labels
    weights = np.where(y == 1, class_1_weight, class_0_weight)
    
    # Compute weighted gradient
    gradient = np.dot(tx.T, weights * errors) / y.shape[0]
...
def ridge_regression (y, tx, lambda_) :
    ...
    w = np.linalg.solve(np.dot(tx.T, tx) + 2 * tx.shape[0] * lambda_ * np.eye(tx.shape[1]), np.dot(tx.T, y))
    loss = compute_mse_loss(y, tx, w)
...
def reg_logistic_regression (y, tx, lambda_, initial_w, max_iters, gamma, weight0 = 1, weight1 = 1) :
    """
    ...
    for i in range(max_iters):
        loss = loss_logistic_regression(y, tx, w, weight0, weight1)
        gradient = calculate_gradient_logistic_regression_weights(y, tx, w, weight0, weight1) + 2 * lambda_ * w
        # print(f"At iteration {i}, loss = {loss}")
        w = w - gamma * gradient
